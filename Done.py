...
class MainWindow(wx.Frame):
        def __init__(self, parent, title):
                ...
                menuAbout = helpmenu.Append(wx.ID_ABOUT, "&About", " Read about the text editor and it's making (F2)")
                 ...
                 #Custom menu
                custommenu = wx.Menu()
                selectString = custommenu.Append(wx.ID_ANY, "&Rename variable", " Search same words")
                extractmethod=custommenu.Append(wx.ID_ANY,"&Extract Method","Pass lines of code into method")
                extractclass=custommenu.Append(wx.ID_ANY,"&Extract Class","Pass common methods of classes to superclass")
                extractcondition=custommenu.Append(wx.ID_ANY,"&Extract Condition","Pass if conditions to boolean variables")
                ...
                menuBar.Append(custommenu, "&Options")
                ...
                # Custom events
                self.Bind(wx.EVT_MENU, self.onButton, selectString)
                self.Bind(wx.EVT_MENU, self.onExtract, extractmethod)
                self.Bind(wx.EVT_MENU, self.onExtractclass, extractclass)
                self.Bind(wx.EVT_MENU, self.onExtractcondition, extractcondition)
                # Key bindings
        ...
        def onExtract(self,e):
                import extractmethod
                reload(extractmethod)
                f = open('preet.txt','r')
                self.control.SetValue(f.read())
                f.close()                       
        ...
        def onExtractclass(self,e):
                ...
                self.control.SetValue(f.read())
                f.close()
        def onExtractcondition(self,e):
                import shit                
                shit.getif(self.control.GetValue())
                f = open('preet.txt','r')
                self.control.SetValue(f.read())
                f.close()
        ...
        def OnOpen(self, e):
                ...
                try:
                        ...
                        if (dlg.ShowModal() == wx.ID_OK):
                                ...
                                f1 = open('preet.txt', 'w')
                                f1.write(self.control.GetValue())
                                f.close()
                                f1.close()
                                f1 = open('variable.txt', 'w')
                                f1.write("")
                                f1.close()
                                f=open('returnvariable.txt','w')
                                f.write("")
                                ...
                                f.close
                                import class1
                                class1.getclass(self.control.GetValue())
                        dlg.Destroy()
                except:
        ...
        def OnButton(self):
                ...
                s=self.text.get(tk.SEL_FIRST, tk.SEL_LAST)
                f=self.text.get("1.0", END)
                s2=f.replace(s,'<b>'+s+'<b>')
